...
src_configure() {
	...
	rust_targets="${rust_targets},\"aarch64-unknown-linux-musl\""
	...
	cat <<- EOF > "${S}"/config.toml
		...
		rpath = false
		musl-root = "/usr/aarch64-unknown-linux-musl/usr"
	EOF
	...
	cat <<- EOF >> "${S}"/config.env
		CFLAGS_aarch64-unknown-linux-musl=-O2 -march=armv8-a+crc -mtune=cortex-a53
	EOF

	cat <<- EOF >> "${S}"/config.toml
		[target.aarch64-unknown-linux-musl]
		cc = "aarch64-unknown-linux-musl-gcc"
		cxx = "aarch64-unknown-linux-musl-c++"
