...
src_prepare() {
	if ! use vanilla; then
		...
		eapply "${WORKDIR}"/vte-0.48.4-command-notify.patch
	fi

	if use termite-patch; then
		#eapply "$FILESDIR"/${PN}-0.48.3-ng/*.patch
		eapply "${FILESDIR}"/vte-ng-0.52.2.patch
	fi

	eapply ${FILESDIR}/allow_alt_in_terminal.patch
