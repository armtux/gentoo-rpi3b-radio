	...
	if [[ ${QT5_MODULE} == qtbase ]]; then
		...
		if [[ ${QT5_MINOR_VERSION} -ge 9 ]]; then
			...
			if tc-is-cross-compiler ; then
				sed -i -e "s|\"\$CFG_HOST_QT_TOOLS_PATH/qmake\" -qtconf \"\$QTCONFFILE\" \"\$relpathMangled\" -- \"\$@\"|& $(qt5_qmake_args) |" -e "s|-qtconf \"\$QTCONFFILE\"||" configure || die
				sed -i -e "s|\$\$f16c_cxx: DEFINES += QFLOAT16_INCLUDE_FAST||" src/corelib/global/global.pri || die
				sed -i -e "s|else: F16C_SOURCES += global/qfloat16_f16c.c||" src/corelib/global/global.pri || die
				sed -i -e "s|\${QT_BUILD_DIR}/bin/moc|/usr/lib64/qt5/bin/moc|" util/integrity/qt.bod || die
				sed -i -e "s|\${QT_BUILD_DIR}/bin/rcc|/usr/lib64/qt5/bin/rcc|" util/integrity/qt.bod || die
				sed -i -e "s|\${QT_BUILD_DIR}/bin/uic|/usr/bin/uic|" util/integrity/qt.bod || die
			fi
		fi
	fi
...
qt5-build_src_install() {
	...
	if [[ ${PN} == qtcore ]]; then
		...
		if tc-is-cross-compiler ; then
			sed -i -e "s|CC = gcc|CC = $(tc-getCC)|" -e "s|CXX = g++|CXX = $(tc-getCXX)|" \
			-e "s|QMAKE_CXXFLAGS =|QMAKE_CXXFLAGS = ${CXXFLAGS}|" -e "s|QMAKE_LFLAGS =|QMAKE_LFLAGS = ${LDFLAGS}|" \
			qmake/Makefile
		fi
...
qt5_foreach_target_subdir() {
	...
	for subdir in "${QT5_TARGET_SUBDIRS[@]}"; do
		...
		if [[ "$@" == emake ]] && [[ ${PN} == qtcore ]] && tc-is-cross-compiler ; then
			sed -i -e "s|usr.*moc|usr/lib64/qt5/bin/moc|" -e "s|usr.*rcc|usr/lib64/qt5/bin/rcc|" src/corelib/Makefile || die
			sed -i -e "s|usr.*moc|usr/lib64/qt5/bin/moc|" mkspecs/modules/qt_tool_moc.pri || die
			sed -i -e "s|usr.*rcc|usr/lib64/qt5/bin/rcc|" mkspecs/modules/qt_tool_rcc.pri || die
			sed -i -e "s|usr.*qlalr|usr/lib64/qt5/bin/qlalr|" mkspecs/modules/qt_tool_qlalr.pri || die
			sed -i -e "s|usr.*qfloat16-tables|usr/lib64/qt5/bin/qfloat16-tables|" mkspecs/modules/qt_tool_qfloat16-tables.pri || die
		elif [[ "$@" == emake ]] && [[ ${PN} == qtgui ]] && tc-is-cross-compiler ; then
			sed -i -e "s|usr.*qvkgen|usr/lib64/qt5/bin/qvkgen|" mkspecs/modules/qt_tool_qvkgen.pri || die
			sed -i -e "s|usr.*qvkgen|usr/lib64/qt5/bin/qvkgen|" src/gui/qvkgen_wrapper.sh || die
			sed -i -e "s|-msse2 ||" src/gui/Makefile || die
			sed -i -e "s|-mssse3 ||" src/gui/Makefile || die
			sed -i -e "s|-msse4.1 ||" src/gui/Makefile || die
			sed -i -e "s|-msse4.2 ||" src/gui/Makefile || die
			sed -i -e "s|-mavx ||" src/gui/Makefile || die
			sed -i -e "s|-mavx2 ||" src/gui/Makefile || die
			sed -i -e "s|-mavx512f ||" src/gui/Makefile || die
			sed -i -e "s|-mavx512er ||" src/gui/Makefile || die
			sed -i -e "s|-mavx512cd ||" src/gui/Makefile || die
			sed -i -e "s|-mavx512pf ||" src/gui/Makefile || die
			sed -i -e "s|-mavx512dq ||" src/gui/Makefile || die
			sed -i -e "s|-mavx512bw ||" src/gui/Makefile || die
			sed -i -e "s|-mavx512vl ||" src/gui/Makefile || die
			sed -i -e "s|-mavx512ifma ||" src/gui/Makefile || die
			sed -i -e "s|-mavx512vbmi ||" src/gui/Makefile || die
		elif [[ "$@" == emake ]] && [[ ${PN} == qtwidgets ]] && tc-is-cross-compiler ; then
			sed -i -e "s|usr.*uic|usr/bin/uic|" mkspecs/modules/qt_tool_uic.pri || die
			sed -i -e "s|usr.*uic|usr/bin/uic|" src/widgets/uic_wrapper.sh || die
			sed -i -e "s|\${QT_BUILD_DIR}/bin/uic|/usr/bin/uic|" util/integrity/qt.bod || die
		elif [[ "$@" == qt5_qmake ]] && [[ ${PN} == qtwayland ]] && tc-is-cross-compiler ; then
			local sed0="s@${WORKDIR}/${PN}-everywhere-src-${PV}/bin/qtwaylandscanner@/usr/lib64/qt5/bin/qtwaylandscanner@"
			local sed0a="s@TARGET.*=./usr/lib64/qt5/bin/qtwaylandscanner@TARGET=../../bin/qtwaylandscanner@"
			local sed1="s@\s\\.\\./\\.\\./bin/qtwaylandscanner@/usr/lib64/qt5/bin/qtwaylandscanner@"
			local sed2="s@\\.\\./\\.\\./\\.\\./\\.\\./\\.\\./bin/qtwaylandscanner@/usr/lib64/qt5/bin/qtwaylandscanner@"
			local sed3="s< QMAKE_LFLAGS_DEBUG= . \\\\\&\\\\\& .{7} -f Makefile< ) \\\\\&\\\\\& find ${WORKDIR} -type f ! -name 'sed*' -print0 | xargs -0 sed -i -e \\\\\\\\\\\"${sed2}\\\\\\\\\\\" -e \\\\\\\\\\\"${sed0}\\\\\\\\\\\" -e \\\\\\\\\\\"${sed0a}\\\\\\\\\\\" \\\\\&\\\\\& \$(MAKE) -f Makefile<"
			local sed3a="s</ \\\\\&\\\\\& .{7} -f Makefile</ \\\\\&\\\\\& find ${WORKDIR} -type f ! -name \\\\\'sed*\\\\\' -print0 | xargs -0 sed -i -e \\\\\\\\\\\"${sed2}\\\\\\\\\\\" -e \\\\\\\\\\\"${sed1}\\\\\\\\\\\" \\\\\&\\\\\& \$(MAKE) -f Makefile<"
			local sed4="s> QMAKE_LFLAGS_DEBUG= . \\\\\&\\\\\& .{7} -f Makefile> ) \\\\\&\\\\\& find ${WORKDIR} -type f ! -name 'sed*' -print0 | xargs -0 sed -E -i -e \\\\\"${sed3a}\\\\\" -e \\\\\"${sed3}\\\\\" -e \\\\\"${sed1}\\\\\" -e \\\\\"${sed0}\\\\\" -e \\\\\"${sed0a}\\\\\" \\\\\&\\\\\& \$(MAKE) -f Makefile>"
			local sed4a="s>/ \\\\\&\\\\\& .{7} -f Makefile>/ \\\\\&\\\\\& find ${WORKDIR} -type f ! -name 'sed*' -print0 | xargs -0 sed -E -i -e \\\\\"${sed3a}\\\\\" -e \\\\\"${sed3}\\\\\" -e \\\\\"${sed1}\\\\\" -e \\\\\"${sed0}\\\\\" -e \\\\\"${sed0a}\\\\\" \\\\\&\\\\\& \$(MAKE) -f Makefile>"
			local sed5="s; \&\& \$(MAKE) -f Makefile; \&\& sed -E -i -e \"${sed4a}\" -e \"${sed4}\" Makefile \&\& \$(MAKE) -f Makefile;"
			local sed5a="s;QMAKE_LFLAGS_RELEASE=.*Makefile install;\) \&\& \$(MAKE) -f Makefile install;"
			sed -i -e "${sed5}" -e "${sed5a}" Makefile || die
		fi
	done
}
...
qt5_base_configure() {
	...
	local conf=(
		...
		$(tc-is-cross-compiler && echo -hostprefix / -external-hostbindir /usr/lib64/qt5/bin -xplatform linux-g++ -device-option CROSS_COMPILE=${CHOST}-)
...
qt5_qmake() {
	...
	local qmakepath=
	if [[ ${QT5_MODULE} == qtbase ]] && !(tc-is-cross-compiler) ; then
