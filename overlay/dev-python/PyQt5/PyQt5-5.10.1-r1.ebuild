...
src_compile() {
	if tc-is-cross-compiler ; then
		sed -i -e "s|CXX.*=.*|CXX = $(tc-getCXX)|" "${WORKDIR}"/${PN}_gpl-${PV}-python*/cfgtest_QtCore.mk || die
		sed -i -e "s|LINK.*=.*|CXX = $(tc-getCXX)|" "${WORKDIR}"/${PN}_gpl-${PV}-python*/cfgtest_QtCore.mk || die
		sed -i -e "s|CXXFLAGS*=.*|CXXFLAGS = ${CXXFLAGS} -pipe -O2 -Wall -W -D_REENTRANT -fPIC \$(DEFINES)|" "${WORKDIR}"/${PN}_gpl-${PV}-python*/cfgtest_QtCore.mk || die
		sed -i -e "s|CC.*=.*|CC = $(tc-getCC)|" "${WORKDIR}"/${PN}_gpl-${PV}-python*/cfgtest_QtCore.mk || die
		sed -i -e "s|CFLAGS*=.*|CFLAGS = ${CFLAGS} -pipe -O2 -Wall -W -D_REENTRANT -fPIC \$(DEFINES)|" "${WORKDIR}"/${PN}_gpl-${PV}-python*/cfgtest_QtCore.mk || die
	fi
